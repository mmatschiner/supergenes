...
cp ../data/ref/gadMor2.MT_genome.fasta ~/tmp/reference.fa
...
cp ../res/fastq/${sample_id}.MT_genome.R1.fastq ~/tmp/reads.fastq
...
# Run mira.
mira manifest.conf > ${sample_id}.MT_genome.log
...
/cluster/software/VERSIONS/mitobim-1.8/MITObim_1.8.pl \
    ...
    -maf ${sample_id}_assembly/${sample_id}_d_results/${sample_id}_out.maf >> ${sample_id}.MT_genome.log

# Copy the fasta file of the last iteration to the output directory.
for i in iteration*
do
    cp -f ${i}/sample_${sample_id}-reference.fa_assembly/sample_${sample_id}-reference.fa_d_results/sample_${sample_id}-reference.fa_out_sample_${sample_id}.unpadded.fasta ${home}/../res/fasta/${sample_id}.MT_genome.fasta
done

# Copy the log file to the log directory.
cp ${sample_id}.MT_genome.log ${home}/../log/mitobim
