...
names_ordered.size.times do |x|
	names_ordered.size.times do |y|
		...
		p1_matrix[x][y] = selected_p1
		p1_matrix[y][x] = selected_p1
		p2_matrix[x][y] = selected_p2
		p2_matrix[y][x] = selected_p2
		p3_matrix[x][y] = selected_p3
		p3_matrix[y][x] = selected_p3
	end
end
...
(d_matrix.size).times do |z1|
	...
	(d_matrix.size).times do |z2|
		...
		if opacity > 0
			...
			# Find the line of the combine file with the three taxa.
			trio_found =  false
			trio_ary = []
			combine_lines.each do |cl|
				cl_ary = cl.split
				if cl_ary[0..2].sort == [p1_matrix[z1][z2],p2_matrix[z1][z2],p3_matrix[z1][z2]].sort
					trio_found = true
					trio_ary = Marshal.load(Marshal.dump(cl_ary))
				end
			end
			if trio_found == false
				puts "ERROR: Trio was not found!"
				exit 1
			end
			if p1_matrix[z1][z2] == trio_ary[0] and p2_matrix[z1][z2] == trio_ary[1] and p3_matrix[z1][z2] == trio_ary[2]
				nBBAA = trio_ary[3].to_f
				nABBA = trio_ary[5].to_f
				nBABA = trio_ary[4].to_f
			elsif p1_matrix[z1][z2] == trio_ary[0] and p2_matrix[z1][z2] == trio_ary[2] and p3_matrix[z1][z2] == trio_ary[1]
				nBBAA = trio_ary[4].to_f
				nABBA = trio_ary[5].to_f
				nBABA = trio_ary[3].to_f
			elsif p1_matrix[z1][z2] == trio_ary[1] and p2_matrix[z1][z2] == trio_ary[0] and p3_matrix[z1][z2] == trio_ary[2]
				nBBAA = trio_ary[3].to_f
				nABBA = trio_ary[4].to_f
				nBABA = trio_ary[5].to_f
			elsif p1_matrix[z1][z2] == trio_ary[1] and p2_matrix[z1][z2] == trio_ary[2] and p3_matrix[z1][z2] == trio_ary[0]
				nBBAA = trio_ary[5].to_f
				nABBA = trio_ary[4].to_f
				nBABA = trio_ary[3].to_f
			elsif p1_matrix[z1][z2] == trio_ary[2] and p2_matrix[z1][z2] == trio_ary[0] and p3_matrix[z1][z2] == trio_ary[1]
				nBBAA = trio_ary[4].to_f
				nABBA = trio_ary[3].to_f
				nBABA = trio_ary[5].to_f
			elsif p1_matrix[z1][z2] == trio_ary[2] and p2_matrix[z1][z2] == trio_ary[1] and p3_matrix[z1][z2] == trio_ary[0]
				nBBAA = trio_ary[5].to_f
				nABBA = trio_ary[3].to_f
				nBABA = trio_ary[4].to_f
			else
			...
			unless trios_reported.include?([p1_matrix[z1][z2],p2_matrix[z1][z2],p3_matrix[z1][z2]])
				puts "#{p1_matrix[z1][z2]}\t#{p2_matrix[z1][z2]}\t#{p3_matrix[z1][z2]}\t#{'%.1f' % nBBAA}\t#{'%.1f' % nABBA}\t#{'%.1f' % nBABA}\t#{'%.3f' % d_matrix[z1][z2]}\t#{'%.4f' % p_matrix[z1][z2]}"
			end
			trios_reported << [p1_matrix[z1][z2],p2_matrix[z1][z2],p3_matrix[z1][z2]]
		end
	end
end
