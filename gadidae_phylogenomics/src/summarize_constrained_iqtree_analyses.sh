# m_matschiner Wed Oct 30 23:51:03 CET 2019

# Get the likelihoods for the three topologies per hypothesis.
for hypothesis in h01 h02
do
    table=../res/tables/constrained_likelihoods_${hypothesis}.txt
    if [ ! -f ${table} ]
    then
        echo -e "gene_id\tbest\tdelta_lik\tlik_t1\tlik_t2\tlik_t3" >> ${table}
        for phylip in ../res/iqtree/alignments/*.phy
        do
            marker_id=`basename ${phylip%.phy}`

            # Test if all three iqtree info files exist.
            t1_file=../res/iqtree_constrained/${hypothesis}/${marker_id}_${hypothesis}_t1.info.txt
            t2_file=../res/iqtree_constrained/${hypothesis}/${marker_id}_${hypothesis}_t2.info.txt
            t3_file=../res/iqtree_constrained/${hypothesis}/${marker_id}_${hypothesis}_t3.info.txt
