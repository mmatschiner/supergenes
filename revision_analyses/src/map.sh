...
do
    ...
    do
        ...
        elif [ "${species_id}_${library_id}" == "Gadmor_LOF1106011L005" ]
        then
	        specimen_id=Gadmor_lfc3
        elif [ "${species_id}_${library_id}" == "Gadmor_LOF1106011L006" ]
        then
	        specimen_id=Gadmor_lfc3
        ...
        then
            sbatch --account=${acct} -o ${out} --partition=bigmem --mem-per-cpu=10G map.slurm ${specimen_id} ${ref} ${bam} ${log} ${fastq1} ${fastq2}
	else
            sbatch --account=${acct} -o ${out} map.slurm ${specimen_id} ${ref} ${bam} ${log} ${fastq1} ${fastq2}
        fi
    done
done
